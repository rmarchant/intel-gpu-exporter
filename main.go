...
func main() {
	...
	if err := Main(ctx, prometheus.DefaultRegisterer, l); err != nil {
		slog.Error("gpu monitor failed to run", "err", err)
		os.Exit(1)
	}
}

func Main(ctx context.Context, r prometheus.Registerer, l *slog.Logger) error {
	l.Info("gpu monitoring starting", "version", version, "addr", *addr)

	cmd, stdout, err := runTop(ctx)
	if err != nil {
		return fmt.Errorf("top failed to start: %w", err)
	}

	l.Debug("top started", "cmd", cmd.String())
	...
	go func() {
		if err := a.Read(stdout); err != nil {
			l.Error("gpu top read failed", "err", err)
		}
	}()
